...
public class GamemodeCommand extends AbstractCommand {
	...
	public GamemodeCommand(EssentialsReloaded essentialsReloaded) {
		super(essentialsReloaded, "gamemode", "essentials.gamemode");
		this.GAMEMODES = List.of("2", "3", "0", "1", "spectator", "creative", "survival", "adventure");
		// And yes, I just wanted to show off my freshly gained and immediately
		// forgotten Regex skills
		...
		if (playerData.isPlayer()) {
			if (args.length >= 1) {
				...
				if (player == null) {
					...
					if (!self && playerData.player().getUniqueId().equals(player.getUniqueId()))
						self = true;

					GamemodeData gamemodeData = gamemode(playerData.player(), player, args[0]);
					if (gamemodeData.noperm)
						sender.sendMessage(essentialsReloaded.messageHandler()
								.getMessage("general.no_perms").build());
					if (gamemodeData.target_exempt)
						sender.sendMessage(essentialsReloaded.messageHandler()
								.getMessage("gamemode.target_exempt")
								.placeholder("player", player.getDisplayName())
								.build());
					if (gamemodeData.changed) {
						playerData.player().sendMessage(essentialsReloaded.messageHandler()
								.getMessage("gamemode."
										+ (self ? "set_own" : "set_other"))
								.placeholder("gamemode", StringUtils
										.capitalize(gamemodeData.gamemode.name()
												.toLowerCase()))
								.placeholder("player", player.getDisplayName())
								.build());
					}
	...
	public List<String> tab(CommandSender sender, Command command, String[] args, PlayerData playerData) {
		if (args.length == 1) {
			...
			StringUtil.copyPartialMatches(args[1], Bukkit.getOnlinePlayers().stream()
					.map(HumanEntity::getName).collect(Collectors.toList()), x);
			return x;
		} else
	...
	private GamemodeData gamemode(Player sender, Player target, String gamemode) {
		...
		if (target.hasPermission(this.permission + ".exempt"))
			return data.exempt();
		if (sender.hasPermission(this.permission + "."
				+ (sender.getUniqueId().equals(target.getUniqueId()) ? "own" : "other") + "."
