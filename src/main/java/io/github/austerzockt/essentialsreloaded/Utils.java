...
public final class Utils {
    ...
    public static class CommandUtils {
        public List<String> listOfMaterials(String input, boolean lowercaseOutput) {
            ...
            List<String> values = Arrays.stream(Material.values()).map(Enum::name).map(s -> { if (lowercaseOutput) return s.toLowerCase(); else return s; }).filter(s -> !s.startsWith(Material.LEGACY_PREFIX)).collect(Collectors.toList());
