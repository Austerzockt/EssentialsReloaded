...
public final class Utils {
	...
	public static class CommandUtils {
		public List<String> listOfMaterials(String input, boolean lowercaseOutput) {
			...
			// Some weird BS with Concurrent Access and the Iterator in
			// StringUtil.copyPartialMatches ... This fixes it, might not be a nice
			// solution, but i am tired and i frankly dont fking care
	...
	public static class PlayerUtils {
		public boolean isOnline(String name) {
			return Bukkit.getOnlinePlayers().stream().filter(s -> s.getName().equalsIgnoreCase(name))
					.findFirst().orElse(null) != null;
		}

		public boolean isOnline(UUID uuid) {
			return Bukkit.getOnlinePlayers().stream().filter(s -> s.getUniqueId().equals(uuid)).findFirst()
					.orElse(null) != null;
		}

		public Player getPlayer(String name) {
			return (isOnline(name) ? Bukkit.getPlayer(name) : null);
		}

		public Player getPlayer(UUID uuid) {
			return (isOnline(uuid) ? Bukkit.getPlayer(uuid) : null);
		}
	}
